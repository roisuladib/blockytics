...
import type { ShibariumDepositsResponse, ShibariumWithdrawalsResponse } from '^types/api/shibarium';
...
import type { UserOp, UserOpsAccount, UserOpsFilters, UserOpsResponse } from '^types/api/userOps';
import type {
...
import type { WithdrawalsCounters, WithdrawalsResponse } from '^types/api/withdrawals';
import type {
...
import type { MarketplaceAppOverview } from '^types/client';
import type { ArrayElement } from '^types/utils';

const marketplaceFeature = getFeaturePayload(config.features.marketplace);
const marketplaceApi =
   marketplaceFeature && 'api' in marketplaceFeature ? marketplaceFeature.api : undefined;
...
export type ResourceFiltersKey<R extends ResourceName> = (typeof RESOURCES)[R] extends {
   filterFields: Array<unknown>;
}
...
type ResourcePathParamName<Resource extends ResourceName> = (typeof RESOURCES)[Resource] extends {
   pathParams: Array<string>;
}
