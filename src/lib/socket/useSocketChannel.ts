...
export function useSocketChannel({
   ...
   useEffect(() => {
      const cleanUpRefs = () => {
         const refs = [onCloseRef.current, onErrorRef.current].filter(Boolean).map(String);
      ...
      return cleanUpRefs;
   }, [isDisabled, onSocketClose, onSocketError, socket]);

   useEffect(() => {
      if (socket === null || isDisabled || !topic) return;
